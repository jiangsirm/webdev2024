...
router.post("/",
    function (req, res) {
        ...
        return res.send(reqBody.name + " get'em !");
    }
)
...
router.delete('/:pokemonName', (req, res) => {
    let pokeName = req.params.pokemonName;

    pokeColors = pokeColors.filter(
        (pokemon) => {
            return pokemon.name !== pokeName
        }
    )

    return res.status(200).send(pokeName + " is Gone!");
})
